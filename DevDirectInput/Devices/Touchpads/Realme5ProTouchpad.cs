...
{
    [Obsolete]
    public class Realme5ProTouchpad : ITouchpad
    {
        ...
        public Realme5ProTouchpad(ITouchpadOptions options)
        {
            Options = options;
        }

        public Realme5ProTouchpad(ITouchpadOptions options, EDevicePurpose purpose)
        {
            Options = options;
            Purpose = purpose;
        }
        ...
        {
            //_timeline.Correct(ticks, startTick);
                ...
                foreach (var touchpadEvent in touchpadEvents)
                {
                    ...
                    switch (touchpadEvent.EventType)
                    {
                        case ETouchpadEventType.Press:
                            update.Add(new RawInputEvent(EEventType.EvAbs, EEventCode.AbsMtTrackingId, (uint)touchpadEvent.Slot));
                            update.Add(new RawInputEvent(EEventType.EvKey, EEventCode.BtnTouch, ETouchState.Press));
                        ...
                        case ETouchpadEventType.Release:
                            ...
                            update.Add(new RawInputEvent(EEventType.EvAbs, EEventCode.BtnTouch, ETouchState.Release));
                            break;
